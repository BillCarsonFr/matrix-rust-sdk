...
use ruma::{
    EventId, OwnedMxcUri, OwnedUserId, RoomId,
    api::client::{
...
impl MatrixDriver {
    ...
    pub(crate) async fn send(
        ...
        Ok(match (state_key, delayed_event_parameters) {
            ...
            (None, Some(delayed_event_parameters)) => self
                .room
                .send_delayed_raw(&type_str, content, delayed_event_parameters)
                .await?
                .into(),
    ...
    pub(crate) async fn send_to_device(
        ...
        let failures = self
            .room
            .client()
            .send_raw_to_device(&event_type.to_string(), messages, room_encrypted)
            .await?;

        // The widget API expects the failures as a stringified [user -> devices] map.
